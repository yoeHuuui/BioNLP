...
result = open("Key_Result_Transform.txt",'r',encoding='utf-8')
tree = open("Link.txt", 'w', encoding= 'utf-8')
...
for i in result:
    ...
    if CountForSentance % 1500 == 0:
        print("the previous progress lies around {:.2f}%".format(CountForSentance / 10570.92))
    try:
        if ' ' in text[3]:
            s=0
